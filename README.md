...
- **プロジェクトカラー**（v1.5 NEW）: プロジェクトごとに10色からカラーを設定可能
...
- **続きを書く**: 文章の続きを自動生成
- **書き直す**: 選択部分をより良い表現に
- **拡張する**: 選択部分を詳しく描写（v1.5改良：参照内容をそのまま出力しないアルゴリズム）
- **目標文字数指定**: 生成する文字数を指定可能（※目安表示）
- **追加指示入力**（v1.5 NEW）: 各AI生成時に追加の指示を入力可能
- **ネガティブプロンプト**（v1.5 NEW）: 「〜しないで」という除外指示を設定可能

### 🔗 参照プロジェクト機能（v1.5 大幅強化）
- 他のプロジェクトをAI生成の参照として選択
- 過去のアイデアやプロットを知識ベースとして活用
- 複数プロジェクトの同時参照に対応
- **フォルダ階層表示**（v1.5 NEW）: プロジェクト一覧でフォルダごとにグループ化表示
- **フォルダ一括選択**（v1.5 NEW）: フォルダ内のプロジェクトを一括で選択/解除
- **参照強度2段階**（v1.5 NEW）:
  - **強**: 内容をそのまま参照して活用
  - **弱**: 参考にして類似のものを出力（インスピレーションとして活用）

### 🧩 構成要素抽出機能（v1.5 NEW）
- 入力された文章から以下の要素を自動抽出：
  - **文体**: 文章のトーン、語り口、文末表現
  - **シナリオ**: ストーリーの流れ、プロット構造
  - **キャラクター設定**: 登場人物の性格、関係性
  - **世界観**: 舞台設定、雰囲気
  - **その他の構成要素**: テーマ、モチーフなど
- 執筆モードのツールバーからアクセス可能
- 抽出結果はモーダルで表示、コピー可能
...
webapp/
├── src/
│   └── index.tsx          # メインアプリケーション（API + ルーティング）約3,000行
├── public/
│   ├── static/
│   │   ├── app.js         # フロントエンドJS（約7,200行）
│   │   ├── logo.png       # アプリロゴ
...
│   └── 0013_add_project_color.sql        # v1.5 プロジェクトカラー
...
- **google_id** (v1.4) - Google OAuth連携用
...
- folder_id, **color** (v1.5 NEW), deleted_at, created_at, updated_at
...
| PUT | `/api/projects/:id/color` | プロジェクトカラー更新（v1.5 NEW） |
...
14. **構成要素抽出**: 執筆モードで文章から文体・シナリオ・キャラ設定を抽出（v1.5 NEW）
...
### v1.5 (2025-05-04)
- ✨ **参照プロジェクト機能の大幅強化**
  - フォルダ階層表示: プロジェクト一覧でフォルダごとにグループ化
  - フォルダ一括選択: フォルダ内のプロジェクトを一括で選択/解除
  - 参照強度2段階: 強（そのまま参照）/ 弱（インスピレーションとして参照）
- ✨ **プロジェクトカラー機能追加**
  - プロジェクト一覧からカラーを設定可能（10色）
  - projectsテーブルにcolorカラム追加
- ✨ **ネガティブプロンプト機能追加**
  - AI生成時に「〜しないで」という除外指示を設定可能
  - 全12言語対応
- ✨ **構成要素抽出機能追加**
  - 文章から文体・シナリオ・キャラ設定などを自動抽出
  - 執筆モードのツールバーからアクセス可能
- ✨ **AI生成時の追加指示入力**
  - 続きを書く/書き直す/拡張するで追加の指示を入力可能
- 🔧 **拡張機能のアルゴリズム改良**
  - 参照内容をそのまま出力しないよう改善
- 🌍 全新機能の12言語翻訳対応
